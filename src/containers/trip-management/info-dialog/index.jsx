...
const InfoDetails = ({
  ...
  const weightFormat = (weight, weight_in) => {
    let num = [];
    if (weight && strictValidString(weight_in) && weight_in === 'KG') {
      const numb = toNumber(weight) / 2.205;
      num.push(parseFloat(numb).toFixed(1));
      return num;
    }
    return weight;
  };
        ...
        <React.Fragment>
          ...
          <Stack mt={3} direction={'row'} display="flex">
            <Box>
              ...
              <Typography style={{ marginLeft: 6 }} className={classes.margin}>
                {weightFormat(
              ...
              {/* {renderSubtitleWithSpace(
                strictValidObjectWithKeys(data.base_patient) &&
                  ...
                  ? toNumber(parseFloat(data.base_patient.weight).toFixed(0)) /
                      2.205 +
                      ' ' +
                      data.base_patient.weight_in
                  : data.base_patient.weight +
          ...
          {(userFeedback?.length > 0 ||
            (reasonData?.length > 0 && data?.completed === true)) && (
            <>
              <Box mt={2}>
                {reasonData?.length > 0 && data?.completed === true && (
                  <Accordion
...
const mapStateProps = (state) => {
  return {
    userFeedback: state.trip?.getFeedback?.get_feedback?.trip_feedbacks || [],
    tripLogs: state.trip?.getTripLogs?.get_triplog?.trip_logs || [],
    isLoad: state.trip.getFeedback.isLoad,
    reasonData: state.trip?.getReasons?.get_reason || [],
    load: state.trip.getReasons.isLoad,
    logsload: state.trip.getTripLogs.logs_load,
  };
};

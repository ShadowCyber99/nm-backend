...
const EditTrips = ({
  ...
  const [accountFieldId, setAccountFieldId] = useState(0);
  ...
  useEffect(() => {
    if (strictValidObjectWithKeys(current_tripData)) {
      setAccountFieldId(current_tripData.account_field_id);
    }
  }, [current_tripData]);
  ...
  const onSubmit = async (val, form) => {
    let result = {};
    if (!strictValidArrayWithLength(questions)) {
      val.question_id = [];
    }
    await setDilogData({
      ...current_tripData,
      ...
      for (let corporateContact of corporateContacts) {
        empty_val.push({
          ...
          contact_id:
            ...
            current_tripData.isCopy === false
              ? current_tripData.base_patient.contact_id
  ...
  useEffect(() => {
    if (strictValidObjectWithKeys(current_tripData) && formType === 'edit') {
      const data = {
        leg_id: current_tripData.leg_id,
        type: formType,
      };
      saveLegId(data);
    } else if (
      ...
      const data = {
        leg_id: dilogData.new_leg_id,
        type: formType,
      };
      saveLegId(data);
    }
  }, [current_tripData, dilogData]);
    ...
    <Grid item xs={12} md={12} lg={12}>
      <Form
        ...
        validate={(values) => {
          ...
          if (!values.weight_in) {
            errors.weight = 'Unit measurement is required';
          }
          ...
          if (strictValidArrayWithLength(values.capability_id)) {
            if (checkCapabilityCombination(capabilityNames)) {
              ...
              if (
                values.weight_in === 'KG' &&
                weight >= errorData.bariatricLargeWeight &&
                !isBariatric
              ) {
        ...
        initialValues={{
          ...
          new_corporate_contact: [],
          account_field_id: accountFieldId,
          corporate_contact:
            strictValidObjectWithKeys(current_tripData) &&
            strictValidArrayWithLength(current_tripData.corporate_contact)
                        ...
                        {({ meta, input }) => (
                          <>
                            <MDSelect
                              ...
                              disabled={
                                (strictValidObjectWithKeys(current_tripData) &&
                                  current_tripData.status === 'completed') ||
                                (strictValidObjectWithKeys(current_tripData) &&
                                  current_tripData.status === 'cancelled') ||
                                (strictValidObjectWithKeys(current_tripData) &&
                                  current_tripData.status === 'noshow') ||
                                (strictValidObjectWithKeys(current_tripData) &&
                                  current_tripData.status ===
                ...
                <Field
                  ...
                  disabled={
                    ...
                    (strictValidObjectWithKeys(current_tripData) &&
                      current_tripData.status === 'noshow') ||
                    (strictValidObjectWithKeys(current_tripData) &&
                      current_tripData.status === 'cancelled_no_billable')
                  }
                />
              </Grid>
              <Divider className={classes.margin} />
...
const mapDispatchToProps = (dispatch) => ({
  ...
  saveLegId: (...params) => dispatch(saveLeg(...params)),
});

export default connect(mapStateProps, mapDispatchToProps)(EditTrips);

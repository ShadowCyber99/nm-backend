...
const AddUnit = ({
  ...
  const unitDriverVehicle = async (values) => {
    if (values.start_time && values.end_time) {
      if (
        strictValidObjectWithKeys(unitData) &&
        strictValidObjectWithKeys(values)
      ) {
        values.unit_id = unitData.unit_id;
      }
      values.status = 'active';
      const my_result = await callDriverVehicle(values);
      if (!strictValidObjectWithKeys(unitData)) {
        const roleArray = [];
  ...
  const required = (value) =>
    !isMaxDate ? undefined : 'Please Enter Valid date';
  const onSubmit = async (values) => {
    if (strictValidObjectWithKeys(unitData) && unitData.unit_id) {
      const data = {
        unit_id: unitData.unit_id,
        values: values,
      };
      const res = await callUpdateUnitApi(data);

      if (res) {
        socket.emit('create_unit', unitData.unit_id);
        setValue(1);
      }
    } else {
  ...
  const buttonName = strictValidObjectWithKeys(unitData)
    ? 'Update Unit'
    : 'Create Unit';
      ...
      <Form
        ...
        validate={(values) => {
          ...
          if (new Date(values.start_time) >= new Date(values.end_time)) {
            errors.end_time =
              'End Time Date Should Be Greater Then Start Time Date';
          }
          if (minutes === 0 && hours === 0) {
            errors.end_time = 'Working Hours Should be greater then 0 Hours';
          }

          if (
            moment(values.end_time) > moment(values.start_time).add(24, 'hours')
          ) {
            errors.end_time = 'Unit should be less then 24 hours';
          }
          if (!moment(values.start_time).isValid()) {
            errors.start_time = 'Please Enter Valid Date';
          }
          if (!moment(values.end_time).isValid()) {
            errors.end_time = 'Please Enter Valid Date';
          }
          if (!values.driver_id) {
        ...
        initialValues={{
          driver_id: strictValidObjectWithKeys(unitData)
            ? unitData.driver_id
            : '',
          attendant_id: strictValidObjectWithKeys(unitData)
            ? unitData.attendant_id
            : '',
          start_time: strictValidObjectWithKeys(unitData)
            ? unitData.start_time
            : null,
          end_time: strictValidObjectWithKeys(unitData)
            ? unitData.end_time
            : null,
          vehicle_id: strictValidObjectWithKeys(unitData)
            ? unitData.vehicle_id
            : '',
        }}
        render={({
          ...
          const minDateTime = startDateTime.setMinutes(startDateTime.getMinutes() + 30)
          
          startDateTime.setMinutes(startDateTime.getMinutes() - 30)
          const maxDateTime = startDateTime.setDate(startDateTime.getDate() + 1)
                          ...
                          <>
                            <MdDatePicker
                              ...
                              onError={(reason, value) => {
                                switch (reason) {
                                  case 'maxTime':
                                    if(moment(value).diff(maxDateTime, 'minutes') === 0){
                                      setIsMaxDate(false);
                                    }
                                    else{
                                      setIsMaxDate(true);
...
export default connect(mapStateProps, mapDispatchToProps)(AddUnit);

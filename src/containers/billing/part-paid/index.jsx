...
import InfoDialog from '../../payment/info-dialog';
...
const PartiallyPaidInvoice = ({
  ...
  const infoDialog = () => {
    return (
      ...
      >
        <DialogContent>
          <BillingInfo data={current} emptyText={'Part Paid'} />
        </DialogContent>
        <DialogActions
  ...
  useEffect(() => {
    socket.on('refresh_payments', () => {
      callAllPaymentFromInvoiceApi(dialogData);
    });
    return () => {
      socket.off('refresh_payments');
    };
  ...
  useEffect(() => {
    selectedInvoiceId.forEach((res) => {
      applyPaymentData.forEach((a) => {
        if (res === a.id && a.isFullyApplied) {
          if (
            Math.sign(totalAmount) === 1 &&
            (Number(totalAmount) >= Number(a.balance) ||
              Number(totalAmount) <= Number(a.balance))
          ) {
            setTotalAmount(
              totalAmount - (Number(a.balance) - Number(a.applied_amount)),
            );
            const applyNewBalance =
              Number(totalAmount) -
              (Number(a.balance) - Number(a.applied_amount));
            updateItem(
              applyPaymentData,
              a.id,
              'applied_color',
            ...
            updateItem(
              ...
              'balance_cp',
              Math.sign(applyNewBalance) === -1
                ? Number(a.balance_cp) - Number(totalAmount)
                : 0,
            );
          } else {
            setTotalAmount(
          ...
          <Stack py={2}>
            {renderContent(
              ...
              strictValidObjectWithKeys(receiveInvoice) &&
                `${receiveInvoice.invoice_id} / ${
                  strictValidObjectWithKeys(receiveInvoice) &&
                  strictValidString(receiveInvoice.created_at)
                    ? formatDate(
            ...
            strictValidString(originalRow.created_at) ? formatDateTime(
            originalRow.created_at .replace('T', ' ') .replace('Z', ' '), ) :
            formatDateTime(new Date()); */}
            {renderContent(
              'Account',
              strictValidObjectWithKeys(receiveInvoice) &&
                strictValidObjectWithKeys(receiveInvoice.corporate_account) &&
                `${
                  strictValidString(receiveInvoice.corporate_account.name) &&
                  receiveInvoice.corporate_account.name
                }${
                  strictValidString(receiveInvoice.cost_center_name)
                    ? ' - ' + receiveInvoice.cost_center_name
                    : ''
                }`,
            )}
            {renderContent(
            ...
            {renderContent(
              'Invoice Amount',
              strictValidObjectWithKeys(receiveInvoice) &&
                `${defaultCurrencyFormat(receiveInvoice.amount)} `,
            )}
          </Stack>
          <Stack py={2}>
            {renderContent(
          ...
          columnDefs={[
            ...
            },
            {
              ...
              accessor: (originalRow, rowIndex) => {
                return (
                  strictValidObjectWithKeys(originalRow) &&
                  `${defaultCurrencyFormat(originalRow.balance_cp)}`
                );
              },
              width: 80,
              rightAlign: true,
              Cell: (props) => (
                <>
                  <Typography>
                    {defaultCurrencyFormat(props.row.original.balance_cp)}
                  </Typography>
                </>
              ),
            },
...
PartiallyPaidInvoice.propTypes = {
...
PartiallyPaidInvoice.defaultProps = {
  isLoad: false,
  loadErr: '',
  message: '',
};
...
const mapStateProps = (state) => {
  return {
    ...
    data: state.billing.validateInvoice.partPaid.result,
...
export default connect(mapStateProps, mapDispatchToProps)(PartiallyPaidInvoice);
